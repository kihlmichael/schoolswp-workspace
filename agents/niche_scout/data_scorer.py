...
class DataScorerAgent(BaseContentAgent):
    ...
    async def run(  # type: ignore[override]
        ...
        for m in scorer_input.niches:
            ...
            lines.append(f"  volume={_fmt(m.volume, 0)} | kd={_fmt(m.kd, 1)} | "
        ...
        metrics_block = "\n".join(lines)
        context_block = (
            f"\nContexte additionnel : {scorer_input.context}"
            if scorer_input.context
            else ""
        )
    ...
    async def run_from_csv(
        ...
        return await self.run(DataScorerInput(
            niches=niches,
