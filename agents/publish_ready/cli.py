...
Usage :
  ...
  python -m agents.publish_ready.cli \\
    --file articles/lms-pilier/v4.md \\
...
def _print_result(result: PublishReadyResult, elapsed: float) -> None:
    ...
    print(_DASH)
    print(
        f"  Publish Score  [{_pbar(result.publish_score)}] "
        f"{result.publish_score}/100  {result.publish_emoji}"
    )
    print(f"  Diagnostic     {result.publish_diagnostic}")
    print(f"  Bloc le plus faible : {result.weakest_module}")
    print(_SEP)
    print("  SCORES PAR MODULE")
    weights = {"SEO Structure": "30%", "Citabilité IA": "25%",
               "Conversion": "25%", "Autorité thème": "20%"}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '    --file articles/lms-pilier/v4.md \\\n'
            '    --keyword "formation en ligne rentable wordpress" \\\n'
            "    --intent décisionnelle --pillar LMS --save-dir articles/lms-pilier/\n\n"
            "  # Audit + auto-fix module le plus faible\n"
            "  python -m agents.publish_ready.cli \\\n"
            '    --file articles/lms-pilier/v4.md \\\n'
            '    --keyword "lms wordpress" \\\n'
            "    --fix-weakest --threshold 85 --save-dir articles/lms-pilier/"
        ),
    )
    ...
    parser.add_argument("--fix-weakest", action="store_true",
                        help=(
    ...
    parser.add_argument("--threshold", type=int, default=85, metavar="N",
                        help="Seuil de publication (défaut : 85)")
    ...
    parser.add_argument("--output", default=None, metavar="FICHIER",
                        help="Fichier unique pour le dashboard unifié")
...
async def main() -> None:
    ...
    if args.save_dir:
        ...
        (save_path / "conversion-audit.md").write_text(
            result.conversion_result.report, encoding="utf-8"
        )
        saved.append("conversion-audit.md")

        (save_path / "topical-audit.md").write_text(
            result.authority_result.report, encoding="utf-8"
        )
