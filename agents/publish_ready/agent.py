...
from agents.conversion_auditor.agent import ConversionAuditResult, ConversionAuditorAgent
...
def _compute_publish_score(
    seo: int, llm: int, conversion: int, authority: int
) -> int:
...
class PublishReadyAgent(BaseContentAgent):
    ...
    async def _fix_weakest(
        ...
        if weakest_name == "SEO Structure":
            ...
            fix_result = await agent.audit_and_fix(
                article=article, keyword=keyword, intent=intent, threshold=90
            )
        ...
        if weakest_name == "Citabilité IA":
            ...
            opt_result = await agent.optimize(
                article=article, keyword=keyword, intent=intent
            )
