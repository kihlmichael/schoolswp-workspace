...
Usage :
  ...
  python -m agents.conversion_auditor.cli \\
    ...
    --objective formation \\
    --inject --save-dir articles/lms-pilier/
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '    --file articles/lms-pilier/v4.md \\\n'
            '    --keyword "lms wordpress" \\\n'
            "    --objective formation --inject --save-dir articles/lms-pilier/"
        ),
    )
...
async def main() -> None:
    ...
    if args.file:
        p = Path(args.file)
        if not p.exists():
            print(f"[conversion-auditor] Fichier introuvable : {p}", file=sys.stderr)
            sys.exit(1)
