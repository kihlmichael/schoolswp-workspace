...
Usage :
  ...
  python -m agents.knowledge_graph.cli --ner-files articles/cache-wp/ner.json articles/lms/ner.json
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Knowledge Graph Global schoolsWP — "
            "cartographie sémantique de l'écosystème + Score Autorité Graph"
        ),
        ...
        epilog=(
            ...
            "  python -m agents.knowledge_graph.cli \\\n"
            "    --ner-files articles/cache-wp/ner.json articles/lms-comparatif/ner.json \\\n"
            "    --output knowledge-graph.md\n\n"
            "  # Combiné : contexte + NER + sortie fichier\n"
            "  python -m agents.knowledge_graph.cli \\\n"
            '    --context "..." \\\n'
            "    --ner-files ner1.json ner2.json ner3.json \\\n"
            "    --output docs/knowledge-graph.md"
        ),
    )
    parser.add_argument(
        "--context",
