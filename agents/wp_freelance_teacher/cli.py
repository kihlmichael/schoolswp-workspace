...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Professeur IA WordPress Freelance schoolsWP — "
            "créer et livrer des sites clients, logique terrain"
        ),
...
async def main() -> None:
    ...
    if args.list_topics:
        ...
        for m in _MISSION_TYPES:
            print(f"  - {m}")
        print(
            "\nUtilise : python -m agents.wp_freelance_teacher.cli "
            '--topic "<sujet>" --client-type "<client>"\n'
        )
        return
