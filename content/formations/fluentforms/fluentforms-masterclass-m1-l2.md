...
**Objectif** : Maitriser la navigation dans le builder pour etre autonome
...
Je prends un champ "Name" dans la sidebar, je le glisse vers le centre. Il apparait. Je prends un champ "Email", je le glisse en dessous. Fait. Je prends un "Textarea" pour le message. En trois glissements, j'ai un formulaire de contact basique.

Si je veux changer l'ordre, je clique sur un champ et je le fais glisser vers le haut ou le bas. La reorganisation est instantanee.
...
Le panneau de droite s'ouvre avec plusieurs sections.

Label : c'est le texte affiche au-dessus du champ. Par defaut "Name", tu peux le changer en "Ton nom complet" ou "Prenom et nom".

Placeholder : le texte grise a l'interieur du champ. Ca guide le visiteur. Par exemple "Jean Dupont".

Required : tu coches cette case si le champ est obligatoire. Un asterisque rouge apparait a cote du label.

Help Text : un petit texte d'aide affiche sous le champ. Utile pour des instructions specifiques.
...
Double-clic sur un champ dans le preview : ouvre directement ses settings.

Clic droit sur un champ : menu contextuel avec dupliquer, supprimer, deplacer.
...
Dans la prochaine lecon, on passe en revue les 35+ types de champs disponibles. Tu vas voir qu'il y en a certains que tu ne soupconnes meme pas. On se retrouve tout de suite.

---

**Points cles** :
- Trois zones : sidebar champs (gauche), preview (centre), settings (droite)
- Quatre onglets : Form Fields, Input Customization, Form Settings, Preview
- Drag-and-drop pour ajouter et reorganiser les champs
- Configuration d'un champ : label, placeholder, required, help text
- Toujours sauvegarder manuellement (pas d'autosave)

**Mots cles SEO** : FluentForms builder interface, FluentForms drag and drop, creer formulaire WordPress FluentForms
