...
La reponse instinctive, c'est de changer des trucs. Modifier le design. Reecrire le titre. Baisser le prix. Le probleme, c'est que sans donnees, tu ne fais pas de l'optimisation. Tu fais de la divination.

Dans cette lecon, on va poser les bases de l'A/B testing : pourquoi c'est indispensable, quoi tester en priorite, et a quel moment c'est pertinent de se lancer.
...
Tu ne le sais pas. Et tu ne le sauras jamais. Parce que tu as change plusieurs variables simultanement.

C'est le piege numero un de l'optimisation sauvage : tu ne peux pas attribuer un resultat a une cause precise. Tu penses que c'est le nouveau design qui convertit mieux, alors qu'en realite c'est la baisse de prix qui a tout fait. Quand tu rechangeras le design plus tard, tu perdras cette information.

L'A/B testing resout ce probleme. Tu crees deux versions de la meme page, identiques sauf sur un point. Tu envoies 50% du trafic sur chaque version. Et tu compares les resultats. C'est methodique, mesurable, et reproductible.
...
Si tu veux tester le titre de ta page checkout, tu crees une variante B qui est strictement identique a la variante A, sauf le titre. Meme design, meme prix, meme CTA, meme mise en page. La seule difference, c'est le titre.

Si les resultats sont differents, tu sais que c'est le titre qui a fait la difference. Pas le design. Pas le prix. Le titre.

Ca demande de la patience. Tu ne peux tester qu'un element a la fois, ce qui signifie que l'optimisation est un processus lent et iteratif. Mais c'est le seul processus qui produit des certitudes au lieu d'hypotheses.
...
Numero 4 : le design et la mise en page. Nombre de colonnes, position des temoignages, couleur du bouton. L'impact existe, mais il est generalement plus faible que les trois premiers. C'est le dernier element a tester.
...
Si ta page checkout recoit 20 visiteurs par semaine, un test prendra 10 a 20 semaines pour etre conclusif. C'est trop long. Le contexte change, les saisons passent, et tes resultats sont pollues par des facteurs externes.
...
Premiere question : est-ce que j'ai assez de trafic ? Minimum 50 visiteurs par semaine sur la page testee. En dessous, on ne teste pas.

Deuxieme question : est-ce que je teste une seule variable ? Si tu changes deux choses, tu ne sauras pas laquelle a fait la difference.

Troisieme question : est-ce que je sais ce que je vais faire du resultat ? Si la variante B gagne, tu la gardes. Si A gagne, tu gardes A. Si c'est egal, tu gardes l'originale. Definis le plan d'action avant de lancer le test, pas apres.

L'A/B testing n'est pas un exercice intellectuel. C'est un outil de decision. Tu testes pour decider, pas pour explorer.
...
Dans la prochaine lecon, on passe a la pratique. Tu vas creer ton premier split test sur une page checkout directement dans CartFlows.

---

## Notes de production

- **Visuels** : schema intuition vs donnees, tableau 1 variable vs multiples, pyramide des priorites, tableau visiteurs necessaires, checklist pre-test
- **Captures d'ecran** : aucune capture CartFlows dans cette lecon (theorie pure)
- **Ton** : pedagogique et direct, appuye sur la methode
- **Duree estimee** : ~8 min a debit normal
- **Transition** : enchaine directement sur L6.2 (creer un split test)
