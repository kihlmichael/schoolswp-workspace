...
Jusqu'ici, tu as vu tes funnels sous forme de liste. Une etape apres l'autre, de haut en bas. Ca fonctionne, mais ca ne te montre pas la vue d'ensemble. Tu ne vois pas les branches, les chemins alternatifs, les connexions entre les etapes. Le Canvas Mode change ca. C'est l'editeur visuel de CartFlows Pro, et c'est comme ca que tu vas planifier tous tes funnels a partir de maintenant.
...
L'interface change. Tu passes de la liste verticale a un espace de travail visuel. Tes etapes apparaissent sous forme de cartes disposees sur le canevas, avec des fleches qui les relient.

Premiere chose a remarquer : chaque carte a une couleur et une icone qui correspond a son type. Landing en bleu, Checkout en vert, Upsell en orange, Downsell en rouge, Thank You en violet. Tu identifies le type de chaque etape sans meme lire le titre.
...
La zone principale, c'est le canevas lui-meme. C'est la que tes cartes sont disposees. Tu peux zoomer avec la molette de la souris, te deplacer en cliquant-glissant sur le fond, et reorganiser les cartes en les faisant glisser a la position que tu veux.

En haut, tu as la barre d'outils. Le bouton "+" pour ajouter une etape, le bouton de zoom, et le bouton pour revenir a la vue liste.
...
Un panneau s'ouvre avec les types d'etapes disponibles : Landing, Checkout, Upsell, Downsell, Thank You, et Optin. Choisis le type, selectionne un template ou pars de zero, et l'etape apparait sur le canevas.

CartFlows la place automatiquement a la suite du flow, mais tu peux la deplacer ou tu veux. Glisse-la entre deux cartes existantes, et les connexions se reorganisent.

Pour supprimer une etape, ouvre le menu contextuel de la carte et choisis "Supprimer". L'etape disparait et les fleches se reconnectent autour.
...
Pour configurer une condition, glisse la sortie "Accepted" de l'Upsell vers la carte Thank You A, et la sortie "Rejected" vers la carte Downsell. Les fleches se tracent. Tu viens de creer un funnel avec des branches conditionnelles, visuellement, en quelques secondes.
...
Ajoute une page Thank You. Connecte la sortie "Accepted" de l'Upsell vers cette Thank You. C'est le chemin "oui".
...
Avant d'ouvrir Gutenberg pour ecrire ta landing page ou configurer ton checkout, pose d'abord toutes les etapes de ton funnel dans le Canvas. Definis les branches. Verifie que le parcours a du sens. Et une fois que la structure est solide, la tu passes a la creation des pages.

C'est comme construire une maison : d'abord le plan, ensuite les murs. Le Canvas Mode, c'est ton plan.

Dans la prochaine lecon, on va justement entrer dans le detail de chaque type d'etape, en commencant par la Landing Page.

## Notes de production

### Visuels ecran
- Capture 1 : Dashboard CartFlows > Flows > bouton Canvas Mode en haut
- Capture 2 : Vue Canvas avec un flow existant (cartes + fleches)
- Capture 3 : Zoom sur une carte (nom, type, icone, menu contextuel)
- Capture 4 : Panneau d'ajout d'etape (bouton "+" → choix du type)
- Capture 5 : Carte Upsell avec les deux sorties "Accepted" / "Rejected"
- Capture 6 : Funnel complet construit en live (Landing → Checkout → Upsell → branches → Thank You)
- Capture 7 : Comparaison vue liste vs Canvas Mode (split screen)

### Animations / overlays
- Fleche animee montrant le parcours client (vert pour "accepte", rouge pour "refuse")
- Highlight des cartes orphelines (etape non connectee)
- Zoom progressif du canevas pour montrer la vue d'ensemble puis le detail

### Avatar HeyGen
- Cadrage buste, fond neutre
- Presente les sections INTRO, transitions entre sections, OUTRO
- Les sections techniques sont en screencast avec voix-off ElevenLabs
