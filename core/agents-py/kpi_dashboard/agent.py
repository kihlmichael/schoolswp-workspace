...
class KpiDashboardAgent(BaseContentAgent):
    ...
    async def run(  # type: ignore[override]
        ...
        pillar_block = ""
        if pillar_scores:
            lines = "\n".join(f"  - {k.upper()}: {v}/100" for k, v in pillar_scores.items())
        ...
        if llm_scores:
            llm_block = (
                ...
                + "\n".join(f"  - {s.get('article', '?')} : global={s.get('global', 'N/D')}/10" for s in llm_scores)
                + "\n\n"
            )
        ...
        roi_block = f"--- PLAN ROI ÉDITORIAL ---\n{roi_content}\n\n" if roi_content and not article_kpis else ""
        cocon_block = f"--- COCON SÉMANTIQUE ---\n{cocon_content}\n\n" if cocon_content else ""
        ...
        missing_block = "Données manquantes :\n" + "\n".join(f"- {m}" for m in missing) + "\n\n" if missing else ""
