...
Usage :
  ...
  python -m agents.kpi_dashboard.cli \\
    --scan-articles content/articles/ \\
    --authority-file audit/piliers/summary.md
"""
...
def compute_ecosystem_score(
    ...
    """
    auth_mean = sum(pillar_scores.values()) / len(pillar_scores) if pillar_scores else 0.0

    roi_values = [a["roi"] for a in article_kpis if a.get("roi") is not None]
    roi_mean_norm = (sum(roi_values) / len(roi_values) / 9.0) * 100 if roi_values else 0.0

    llm_globals = [s["global"] for s in llm_scores if s.get("global") is not None]
    llm_mean_norm = (sum(llm_globals) / len(llm_globals)) * 10 if llm_globals else 0.0

    score = auth_mean * 0.3 + roi_mean_norm * 0.3 + seo_growth * 0.2 + llm_mean_norm * 0.2
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="kpi-dashboard",
        description=("Dashboard KPI Éditorial schoolsWP — Score Écosystème + 5 vues de pilotage stratégique"),
        ...
        epilog=(
            ...
            "  3. article_pipeline (save-dir) → content/articles/<slug>/\n"
...
async def main() -> None:
    ...
    eco = compute_ecosystem_score(pillar_scores, article_kpis, llm_scores, args.seo_growth)
    print(
