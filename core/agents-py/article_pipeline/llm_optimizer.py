...
_SYSTEM = """Tu es un expert LLM-SEO de schoolsWP, spécialisé en optimisation pour citations IA
...
class LlmOptimizerAgent(BaseContentAgent):
    """
    Agent LLM-SEO du pipeline article — optimisation citations IA de la V2.

    Transforme la V2 en V3 LLM-ready :
    - Paragraphes extractibles et autonomes
    - Encadrés "Définition claire" (2-4)
    - Section "Réponse rapide" optimisée Featured Snippet (≤ 50 mots)
    - Section "Ce qu'il faut retenir" (synthèse décisionnelle)
    - Tableau comparatif si pertinent
    - Évaluation Indice Citation IA (4 dimensions + global après ---llm-scores---)
    """

    name = "pipeline-llm-optimizer"
    ...
    async def run(  # type: ignore[override]
        ...
        """
        Produit la V3 LLM-optimisée à partir de la V2.

        Args:
            article_v2: Texte complet de la V2 (sortie de l'éditeur)
            keyword:    Mot-clé principal (pour ancrer l'optimisation)

        Returns:
            Article V3 en markdown + bloc ---llm-scores--- avec 4 dimensions + global.
        """
        user_message = (
            f"Mot-clé principal : {keyword}\n\n"
            f"--- ARTICLE V2 ---\n\n{article_v2}\n\n"
            "Produis directement la V3 LLM-optimisée."
        )
