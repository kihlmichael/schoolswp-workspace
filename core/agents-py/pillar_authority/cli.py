...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=("Index d'Autorité par Pilier schoolsWP — score /100 + diagnostic + plan d'action"),
    ...
    mode_group.add_argument(
        ...
        metavar="PILIER",
        help=(f"Pilier à analyser : {' | '.join(pillar_choices)}. Incompatible avec --all."),
    )
    mode_group.add_argument(
        "--all",
        action="store_true",
...
async def main() -> None:
    ...
    # ──────────────────────────────────────────────
        ...
        for pillar_key in PILLARS:
            key, text, score = await analyze_pillar(agent, pillar_key, args.context, graph_content)
