...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="PILIER",
        help=(f"Pilier de focus : {' | '.join(_PILLAR_CHOICES)}. Si absent, plan global tous piliers."),
    )
    parser.add_argument(
        "--context",
        default=None,
...
async def main() -> None:
    ...
    data_loaded = sum(1 for x in [graph_content, authority_content, cocon_content] if x)
    ...
    if total > 0:
        print(
            f"  ✓ {total} articles — 🔥 {stats['A']}A  🟡 {stats['B']}B  🔵 {stats['C']}C",
            flush=True,
        )
    if top:
