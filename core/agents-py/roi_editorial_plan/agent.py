...
class RoiEditorialPlanAgent(BaseContentAgent):
    ...
    async def run(  # type: ignore[override]
        ...
        context_block = f"Contexte éditorial :\n{context}\n\n" if context else ""

        articles_block = ""
        if existing_articles:
            articles_list = "\n".join(f"- {a}" for a in existing_articles)
            articles_block = f"Articles déjà publiés (à ne pas dupliquer) :\n{articles_list}\n\n"

        graph_block = f"--- KNOWLEDGE GRAPH schoolsWP ---\n{graph_content}\n\n" if graph_content else ""

        authority_block = f"--- SCORES D'AUTORITÉ PAR PILIER ---\n{authority_content}\n\n" if authority_content else ""

        cocon_block = f"--- COCON SÉMANTIQUE ---\n{cocon_content}\n\n" if cocon_content else ""

        count_block = f"Génère exactement **{count}** idées d'articles.\n\n" if count != 15 else ""
