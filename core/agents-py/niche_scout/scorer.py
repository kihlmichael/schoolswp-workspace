...
_SYSTEM = """Tu es un module de scoring SEO analytique pour schoolsWP.
...
class NicheScorerAgent(BaseContentAgent):
    """
    Agent de scoring SEO basé sur la formule schoolsWP.

    Prend une liste de niches candidates et calcule pour chacune un score /10
    à partir de 5 variables normalisées :
      Volume_norm + Low_Competition + Overlap_schoolsWP + Authority_advantage + Longtail_signal

    Produit un classement reproductible avec calculs explicites.
    """
    ...
    async def run(  # type: ignore[override]
        ...
        """
        Score un ensemble de niches candidates selon la formule schoolsWP.

        Args:
            niches:            Liste des niches à scorer
                               (ex: ["Tutor LMS + FluentCRM", "CRM WordPress avancé"])
                               Chaque entrée peut être un simple nom ou une description
                               plus précise : "Niche : X / requêtes : Y, Z / contexte : ..."
            schoolswp_topics:  Thèmes existants de schoolsWP pour calculer l'overlap
                               (ex: "automatisation n8n, FluentCRM, Tutor LMS, SEO WordPress")
                               Si absent → utilise les thèmes forts par défaut
            context:           Contexte additionnel sur le positionnement ou les données
                               disponibles (ex: "DA schoolsWP estimé à 25", "focus e-commerce")

        Returns:
            Tableau de scoring complet en markdown avec calculs explicites.
        """
        niches_block = "\n".join(f"- {n}" for n in niches)

        topics_block = (
            f"\nThèmes existants de schoolsWP (pour calcul overlap) :\n{schoolswp_topics}" if schoolswp_topics else ""
        )
        context_block = f"\nContexte additionnel : {context}" if context else ""

        user_message = (
            f"Niches candidates à scorer ({len(niches)}) :\n"
            f"{niches_block}"
            f"{topics_block}"
            f"{context_block}\n\n"
            "Applique la formule de scoring schoolsWP sur chaque niche "
