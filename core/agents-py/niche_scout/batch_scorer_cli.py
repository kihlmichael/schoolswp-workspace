...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '    --context "Budget : 4 content/articles/mois. Priorité : leads freelances." \\\n'
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Ex: 'DA ~25, budget 4 content/articles/mois, focus freelances WordPress'"
        ),
    )
    parser.add_argument(
        "--output",
...
async def main() -> None:
    ...
    if len(niches) < _MIN_NICHES:
        print(
            f"[batch-scorer] Erreur : minimum {_MIN_NICHES} niches requises (reçu : {len(niches)}).",
            file=sys.stderr,
        )
        sys.exit(1)
