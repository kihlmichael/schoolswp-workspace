...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="cocon-builder",
        description=(
            "Générateur de Cocon Sémantique schoolsWP — architecture 3 niveaux + scoring + maillage + priorités A/B/C"
        ),
...
async def main() -> None:
    ...
    if satellites > 0:
        print(
            f"  ✓ {satellites} satellites générés — 🔴 {prio_a}A  🟡 {prio_b}B  🟢 {prio_c}C",
            flush=True,
        )
    print("", flush=True)
