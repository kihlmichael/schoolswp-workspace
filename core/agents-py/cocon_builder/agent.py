...
# Piliers supportés — doit rester cohérent avec pillar_authority.agent.PILLARS
...
_SYSTEM = """Tu es un stratège SEO expert en architecture de cocon sémantique, spécialisé dans l'écosystème WordPress.
...
class CoconBuilderAgent(BaseContentAgent):
    """
    Agent Générateur de Cocon Sémantique schoolsWP.

    Produit un cocon en 3 niveaux à partir d'un pilier :
    - Niveau 1 : Page Pilier (1)
    - Niveau 2 : Sous-clusters (3-6)
    - Niveau 3 : Articles Satellites (8-20) avec scoring /12 → Priorité A/B/C

    S'intègre avec les outputs de KnowledgeGraphAgent et PillarAuthorityAgent.
    """
    ...
    async def run(  # type: ignore[override]
        ...
        """
        Génère le cocon sémantique complet pour un pilier schoolsWP.

        Args:
            pillar:             Nom du pilier à développer
                                (ex: "LMS WordPress", "SEO WordPress")
            context:            Contexte éditorial actuel du pilier : articles existants,
                                angles couverts, lacunes, cible audience
                                (ex: "12 articles LMS publiés, focus Tutor LMS,
                                      audience freelances WordPress, aucun tunnel LMS gratuit")
            graph_content:      Output de KnowledgeGraphAgent (.md) — optionnel.
                                Enrichit avec les entités et relations réelles.
            authority_content:  Output de PillarAuthorityAgent pour ce pilier (.md) — optionnel.
                                Aligne les priorités sur les zones faibles identifiées.

        Returns:
            Cocon sémantique complet en markdown : 3 niveaux + scoring + maillage + diff schoolsWP.
        """
        context_block = f"Contexte éditorial actuel sur ce pilier :\n{context}\n\n" if context else ""

        graph_block = f"--- KNOWLEDGE GRAPH schoolsWP ---\n{graph_content}\n\n" if graph_content else ""

        authority_block = f"--- SCORE D'AUTORITÉ DU PILIER ---\n{authority_content}\n\n" if authority_content else ""

        user_message = (
            f"Pilier à développer : **{pillar}**\n\n"
            f"{context_block}"
            f"{authority_block}"
            f"{graph_block}"
            "Génère le cocon sémantique complet selon la structure définie."
        )
