...
from agents.topical_authority.agent import TopicalAuthorityAgent, TopicalAuditResult
...
def _build_factory_report(result: ContentFactoryResult) -> str:
    ...
    lines = [
        f"# schoolsWP Brain — Rapport Factory",
...
class ContentFactoryAgent(BaseContentAgent):
    ...
    async def generate_and_audit(
        ...
        strategy = await brain.run(
            keyword=keyword, intent=intent, pilier=pillar or ""
        )
