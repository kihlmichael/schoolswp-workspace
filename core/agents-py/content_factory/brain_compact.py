...
Usage CLI :
    python -m agents.content_factory.brain_compact \\
      ...
      --intent comparative \\
      --pillar LMS \\
      --save-dir content/articles/lms-compact/

Utilisation n8n (HTTP Request → Claude API) :
    Voir content/docs/brain-api-compact.md pour le body JSON complet.
"""
...
class CompactBrainResult:
    ...
    def composite_score(self) -> int:
        ...
        scores = [s for s in [self.seo_score, self.conversion_score, self.authority_score] if s > 0]
...
class CompactBrainAgent(BaseContentAgent):
    ...
    async def run(  # type: ignore[override]
        ...
        raw = await self.call_llm(user_message, max_tokens=5000)
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        prog="brain-compact",
        description=("schoolsWP Brain Compact — 1 LLM call (~30s). Article + LLM blocks + CTA + scores + cluster."),
        ...
        epilog=(
            ...
            "    --pillar CRM --objective formation --save-dir content/articles/crm-compact/"
        ),
    )
    ...
    parser.add_argument("--topic", default=None, metavar="TITRE_H1", help="Titre H1 si déjà défini (sinon auto-généré)")
    ...
    parser.add_argument("--angle", default=None, metavar="ANGLE", help="Angle différenciant schoolsWP (optionnel)")
    parser.add_argument(
        "--save-dir",
        default=None,
        metavar="DOSSIER",
    ...
    parser.add_argument("--output", default=None, metavar="FICHIER", help="Fichier unique pour l'article")
    parser.add_argument("--json", action="store_true", help="Afficher le résultat en JSON (compatible API)")
