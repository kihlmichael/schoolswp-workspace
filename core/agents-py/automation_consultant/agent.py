...
_SYSTEM = """Tu es un consultant senior en automatisation WordPress travaillant pour schoolsWP.
...
class AutomationConsultantAgent(BaseContentAgent):
    """
    Agent consultant en automatisation WordPress schoolsWP.

    Conçoit des systèmes automatisés (tunnels, CRM, onboarding, workflows)
    pour des freelances et solopreneurs — simplicité et ROI avant tout.
    """
    ...
    async def run(  # type: ignore[override]
        ...
        """
        Génère un plan d'architecture d'automatisation WordPress.

        Args:
            objective:   Ce qu'on veut automatiser (ex: "tunnel de vente formation en ligne")
            tools:       Stack imposée ou souhaitée (ex: ["FluentCRM", "WooCommerce", "n8n"])
            constraints: Contraintes (budget, niveau technique, volume, hébergeur...)

        Returns:
            Plan complet en markdown + meta tags après ---meta---.
        """
        tools_block = ""
        if tools:
            tools_list = ", ".join(tools)
            tools_block = f"\nStack imposée ou souhaitée : {tools_list}"
        ...
        user_message = (
            ...
            f"{tools_block}"
            f"{constraints_block}\n\n"
            "Conçois le système complet selon la structure et les contraintes définies."
        )
