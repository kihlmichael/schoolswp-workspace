...
def generate_part1():
    ...
    # Prompt système et utilisateur avec Few-Shot Prompting pour la partie 1
...
Tâche : Rédige uniquement la PREMIÈRE GRANDE PARTIE (le contexte business et l'importance d'un CRM).
...
5. TRANSITION : Termine cette partie par une phrase de transition naturelle vers la partie suivante (les frictions cloud vs local).
...
Consignes de contenu pour cette partie (Contexte & Importance d'un CRM) :
- Titre H2 : Trouve un titre court et percutant dans le style schoolsWP (ex: "Le suivi client : le point de rupture de ton activité").
- Contenu : 
  * Explique pourquoi le CRM (Customer Relationship Management) n'est pas réservé aux multinationales. C'est le cœur d'activité d'un freelance ou d'un solopreneur.
  * Donne un exemple concret et douloureux : chercher pendant 20 minutes un email crucial au milieu de 500 messages non lus, ou oublier de recontacter un prospect chaud qui était prêt à signer un devis à 2000€.
  * Montre comment ce manque d'organisation te fait perdre de l'argent de manière invisible mais continue.
  * Transition : Annonce que pour résoudre ce problème, on a tendance à se jeter sur les solutions cloud populaires, mais que cela apporte de nouvelles frictions complexes.

Rédige maintenant cette première partie de l'article (respecte scrupuleusement la limite de 250-350 mots et le TUTOIEMENT) :"""
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            print("\n" + "="*50)
            print("ARTICLE DE RÉFÉRENCE schoolsWP — PREMIÈRE PARTIE GÉNÉRÉE")
            print("="*50 + "\n")
            print(text)
            print("\n" + "="*50)
