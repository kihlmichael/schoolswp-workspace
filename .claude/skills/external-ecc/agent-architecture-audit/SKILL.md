...
description: "(import ECC v2.0.0-rc.1 du 2026-05-24 - manuel only - ne pas auto-declencher - voir bandeau)"
...
> **Statut : import ECC manuel-only** (2026-05-24, passe 2 ECC v2.0.0-rc.1, commit 1e8c7e7).
> Frontmatter gutte pour eviter l'auto-trigger. A activer manuellement avant chaque release de pipeline content_factory.cli, article_pipeline.cli, ou tout sub-agent ajoute a .claude/agents/. Complementaire a silent-failure-hunter (agent) et harness-optimizer (agent) deja en place.
> Source : projects/everything-claude-code/skills/agent-architecture-audit/. Origin field "oh-my-agent-check" = mention, pas de dependance code externe.
> Description originale : Full-stack diagnostic for agent and LLM applications. Audits the 12-layer agent stack for wrapper regression, memory pollution, tool discipline failures, hidden repair loops, and rendering corruption.
