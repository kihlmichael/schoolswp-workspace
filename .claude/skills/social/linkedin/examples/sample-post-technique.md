...
**Auteur** : Michael KIHL
**Ton** : tutoiement, professionnel, technique accessible
...
74 % des sites WordPress chargent en plus de 3 secondes. Le tien aussi, probablement.
...
2. Passer les images en WebP avec chargement differe. WordPress 6.9 gere nativement le format WebP a l'upload. Combine avec `loading="lazy"` sur les images below-the-fold (deja actif par defaut depuis WP 5.5), le LCP est passe de 3.1s a 1.4s.

3. Precharger la police principale. Une seule ligne dans le `<head>` : `<link rel="preload" href="police.woff2" as="font" crossorigin>`. Resultat : suppression du flash de texte invisible (FOIT) et 200ms gagnes sur le First Contentful Paint.
...
Si tu veux aller plus loin, j'ai documente le processus complet avec les mesures avant/apres sur schoolsWP.com.

---

## Notes de conformite

- **Tutoiement** : oui (BRAND_RULES regle 5, applique partout schoolsWP sans exception)
...
- **Caracteres typographiques** : pas d'em-dash ni d'en-dash (BRAND_RULES regle 13bis)
