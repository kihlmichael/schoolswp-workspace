...
### ⚠️ CSS selector gotcha

At runtime, `deck_stage.js` **moves every `<section>` into an inner `<div>`** (the scaled canvas). After init the DOM looks like:

```html
<deck-stage>
  <div>                     <!-- injected canvas, holds the scale() transform -->
    <section data-screen-label="01">…</section>
...
This means the obvious CSS rule **silently breaks** once the component boots:

```css
/* ❌ Stops matching after connectedCallback — sections are no longer direct children */
deck-stage > section { width: 1920px; height: 1080px; padding: 96px 128px; }
```

Use one of these instead:

```css
/* ✅ Descendant selector — works before and after init */
deck-stage section { width: 1920px; height: 1080px; padding: 96px 128px; }

/* ✅ Or target the injected attributes directly */
section[data-screen-label] { width: 1920px; height: 1080px; padding: 96px 128px; }
```

Typical failure mode if you get this wrong: slides render at natural content height (500–800px) instead of 1080px, leaving a black letterbox band at the bottom while scale() still thinks the canvas is full size. `browser_evaluate` the current section's `offsetHeight` to confirm (1080 = OK, anything less = selector mismatch).
