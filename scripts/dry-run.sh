...
#
#  Usage : bash scripts/dry-run.sh [--force]
...
if ! curl -sf -u "$N8N_USER:$N8N_PASS" "$N8N_HOST/healthz" > /dev/null 2>&1; then
  ...
  echo "  Vérifier : docker compose -f n8n/docker-compose.yml ps"
  exit 1
fi
...
if [ -z "$WORKFLOW_ID" ]; then
  ...
  echo "  Importer d'abord : n8n/workflows/audit-monthly.json"
  echo "  Via UI : Settings → Import from file"
  echo "  Via CLI : n8n import:workflow --input=n8n/workflows/audit-monthly.json"
  exit 1
fi
...
curl -sf -u "$N8N_USER:$N8N_PASS" \
  -X PATCH "$N8N_HOST/api/v1/workflows/$WORKFLOW_ID" \
...
EXEC_RESPONSE=$(curl -sf -u "$N8N_USER:$N8N_PASS" \
  -X POST "$N8N_HOST/api/v1/workflows/$WORKFLOW_ID/execute" \
  -H "Content-Type: application/json" \
...
echo "  PDF   : ./artifacts/audit-YYYY-MM.pdf"
