#!/usr/bin/env python3
"""Push FR->EN translations to post 54709 (fluent-forms /en/).

Same strtr pattern as wp_push_flyingpress_body_343156.py but for post 54709.
Translates figcaptions + alt= + title= attributes.
...
PAIRS = ROOT / "content" / "audits" / "fluent-forms-en" / "2026-05-12" / "payload-pairs.json"
POST_ID = 54709
...
def build_replacements(pairs):
    """Each pair -> 3 apostrophe variants for robustness."""
    repl = {}
    for fr, en in pairs:
        repl[fr] = en
...
def main():
    ...
    print(f"[info] Expanded to {len(replacements)} replacement keys")
    ...
    if not raw:
        print("[error] content.raw missing")
        sys.exit(1)
    ...
    if new_raw == raw:
        print("[error] no substitution happened - aborting POST")
        sys.exit(2)
    ...
    if status == 200:
        ...
        if sha_server == sha_after_local:
            ...
            print("[info] sha1 differs from local - WP normalized something")
    print(f"[done] post {POST_ID} translation pushed")
